...
function Regions_De_Confiance(algo,f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    while flag == 0
        ...
        if algo == "cauchy"
            ...
            sk = Gradient_Test(gk,Hk,[deltak,max_iter,Tol_rel])
        end
        """le ratio gho_k"""
        ...
        decroissance = 1/2*(norm(gradf(xk))*min(deltak,norm(gradf(xk))/norm(hessf(xk))))
        println(decroissance)
        if (nb_iters == 5)
            break
        end
        xk = xk1
    ...
    return xmin, fxmin, flag, nb_iters, L
end
