...
public class Searcher implements Runnable {
    ...
    public void run() {
        ...
        Tuple treq = linda.read(new Tuple(Code.Request, UUID.class, String.class));//cherche un requete pour la traiter
        UUID reqUUID = (UUID)treq.get(1);
        String req = (String) treq.get(2);
        Tuple tv;
        System.out.println("Looking for: " + req);

        while ((tv = linda.tryTake(new Tuple(Code.Value, String.class))) != null) {//recherhcer de la valeur
            ...
            if (dist<=0) {break;} // la dist ne peut pas etre négatif (ceci signifie qu'on a trouvé la valeur cherchée)
        }

        linda.write(new Tuple(Code.Searcher, "done", reqUUID)); // indique que la recherche est fini
    }
